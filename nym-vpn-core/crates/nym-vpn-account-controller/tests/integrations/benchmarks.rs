...
/// Performance benchmarks for account controller operations.
/// These tests measure the performance of various AC operations to ensure
/// they meet acceptable performance criteria.
...
async fn dhcp_scenario_performance_benchmark() -> anyhow::Result<()> {
    ...
    // Verify DHCP scenarios are handled efficiently (< 200ms average)
    assert!(
        avg_time_per_scenario.as_millis() < 200,
...
#[tokio::test]
async fn concurrent_operations_benchmark() -> anyhow::Result<()> {
    let mut test_bench = TestBench::new().await?;
    ...
    // Simulate concurrent network and tunnel state changes
    let handles = (0..10).map(|_| {
        let mut test_bench = test_bench.clone();
    ...
    // Verify concurrent operations complete within reasonable time (< 5s)
    assert!(
