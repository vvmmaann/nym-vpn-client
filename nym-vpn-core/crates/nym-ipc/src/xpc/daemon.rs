...
use crate::{
    authentication::{self, SigningRequirements, Transport},
    xpc::{
        common::{
...
impl XpcService {
    pub(crate) fn spawn(
        ...
        let signing_requirement = if self_is_signed(&signing_requirements.daemon_req) {
            tracing::debug!("Daemon will do code signing verification for clients");
            Some(signing_requirements.client_req)
        } else {
...
pub fn incoming(
    ...
    let xpc_service = XpcService::spawn(signing_requirements, shutdown_token)?;
