...
impl From<&Error> for SplitTunnelErrorCause {
    fn from(value: &Error) -> Self {
        match &*value.inner {
            InnerError::Process(error) => Self::from(error),
            _v if _v.is_offline() => Self::IsOffline,
            _ => Self::Other,
        }
    }
}
...
enum Message {
    ...
    GetExcludedProcesses {
        result_tx: oneshot::Sender<SplitTunnelExcludedProcessList>,
    },
}
    ...
    async fn handle_message(&mut self, message: Message) {
        match message {
            ...
            Message::SetExcludePaths { result_tx, paths } => {
                let result = self.state.set_exclude_paths(paths).await;
            ...
            Message::GetExcludedProcesses { result_tx } => {
                let excluded_processes = if let Some(process_monitor) = self.state.process_monitor()
                {
                    ...
                    SplitTunnelExcludedProcessList::default()
                };
    ...
    async fn set_tunnel_inner(
        ...
        match self {
            ...
            State::Active {
                ...
                tracing::debug!("Updating split tunnel device");
