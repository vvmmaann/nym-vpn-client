...
impl LazySocks5 {
    ...
    async fn ensure_backend_started(&self) -> Result<(), LazySocks5Error> {
        ...
        // When dVPN is connected, use entry gateway to ensure firewall compatibility.
        // The firewall rules include thes entry gateway endpoints, so using the same gateway
        ...
        if let TunnelState::Connected { connection_data } = tunnel_state {
            ...
            info!(
                "VPN is connected to entry gateway {}, configuring mixnet client to use same gateway for firewall rules",
                entry_gateway_id
            );
    ...
    async fn monitor_tunnel_state(&self) {
        ...
        loop {
            ...
            // Check if we're connected in dVPN mode
            ...
            if dvpn_available && !last_dvpn_available {
                ...
                // vpn connected in dVPN mode - if mixnet client is running, shut it down
                // so it will be recreated with vpns entry gateway on next connection
