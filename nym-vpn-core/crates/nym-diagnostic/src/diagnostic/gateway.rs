...
impl GatewayDiagnostic {
    pub async fn run_diagnostic(
        ...
        let gateway_id_key = ed25519::PublicKey::from_base58_string(gateway_id)?;
        let gateway_client = Self::setup_gateway_client(network).await?;
        ...
        let gateway = match Self::lookup_gateway(&gateway_client, &gateway_id_key).await {
            Ok(maybe_gateway) => {
                ...
                match maybe_gateway {
                    ...
                    None => return Ok(gateway_report),
                }
            }
            Err(e) => {
                gateway_report.gateway = DiagnosticResult::from_err(e);
                return Ok(gateway_report);
            }
        };
    ...
    async fn ws_connection_test(
        ...
        (WebSocketStream<TcpStream>, Response<Option<Vec<u8>>>),
        tokio_tungstenite::tungstenite::Error,
    > {
        let endpoint = format!("ws://{ip}:{port}");
        tokio_tungstenite::client_async(endpoint, tcp_stream).await
    }

    async fn ws_request_test(mut ws_stream: WebSocketStream<TcpStream>) -> anyhow::Result<String> {
        ...
        let response = ws_stream
            .next()
            .await
            .ok_or(anyhow::anyhow!("WS stream closed"))??;
...
pub struct GatewayReport {
    gateway: DiagnosticResult<Option<nym_vpn_lib_types::Gateway>>,

    #[serde(skip_serializing_if = "Option::is_none")]
    tcp: Option<DiagnosticResult<()>>,

    #[serde(skip_serializing_if = "Option::is_none")]
    websocket: Option<DiagnosticResult<()>>,

    #[serde(skip_serializing_if = "Option::is_none")]
    websocket_request: Option<DiagnosticResult<String>>,
}
