...
function computeMaxMtu(
  ...
  if (vpnMode === 'wg') {
    ...
    return isMobileLike ? MIN_IPV6_MTU : ETHERNET_V2_MTU - WG_TUNNEL_OVERHEAD * 2;
  }
    ...
    (next: number | null) => {
      ...
      timerRef.current = setTimeout(async () => {
        ...
        try {
          ...
          dispatch({ type: 'set-mtu', mtu: previous });
          push({
            message: t('advanced-networking.mtu.errors.failed'),
            close: true,
            type: 'error',
          });
        }
    ...
    <PageAnim className="h-full flex flex-col mt-2 gap-6 select-none">
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          {enabled ? (
            <div>
              <div className="flex justify-between text-sm text-iron dark:text-bombay mb-2">
                <span>{minMtu}</span>
